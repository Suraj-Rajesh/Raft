...
class RaftService(rpyc.Service):
    ...
    nodes               = int(config_reader.getTotalNodes())
    # Election timeout timer to be between, T to 2T (random)
    timeoutLower        = int(config_reader.electionTimeoutPeriod())
    peers               = config_reader.get_peer_servers(RaftService.server_id, RaftService.nodes)
    ...
    def voteMe():
        ...
        for peer in RaftService.peers:
            try:
               ...
               if peerConnection not None:
                   peerConnection.requestVotes()

    @staticmethod
    def connect(peer):
        try:
            connection = rpyc.connect(peer[0], peer[1], config = {"allow_public_attrs" : True})
